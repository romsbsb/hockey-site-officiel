...
try {
    ...
    // 1. Liste des matchs
    $reqMatchs = $ConnexionBDD->query("SELECT id_match, adversaire, statut, date_match FROM Matchs ORDER BY date_match DESC");
    $liste_matchs = $reqMatchs->fetchAll(PDO::FETCH_ASSOC);

    if (empty($liste_matchs)) {
        die("Aucun match trouvé dans la base de données. Veuillez en ajouter un.");
    }
    ...
    if (!$match_actuel) {
        foreach($liste_matchs as $m) {
            if($m['date_match'] == $aujourdhui) { $match_actuel = $m; break; }
        }
    }
    if (!$match_actuel) { $match_actuel = $liste_matchs[0]; }

    // 3. Statut des votes (Ouverts de 10h00 à 17h00 les jours de match)
    $votes_ouverts = false;
    if ($match_actuel['date_match'] == $aujourdhui && $match_actuel['statut'] != 'Clôturé') {
    ...
    // 4. Données joueurs
    $req = $ConnexionBDD->query("SELECT id_joueur, nom, prenom, numero, poste, photo, total_motm FROM Joueurs ORDER BY numero ASC");
    $tous_les_joueurs = $req->fetchAll(PDO::FETCH_ASSOC);

    $liste_titulaires = [
        'Matthias Renouf', 'Xavier Rousselet', 'Robin Mars', 'Kenny Burawudi',
        'Timothy (C) Pihouee', 'Alexandre Hardy','Romeo Stab', 'Oscar Lagrange', 
        'Aurélien.H Hardy', 'Aurélien.B Berne', 'Maxime Lagrange'
    ];
    ...
    foreach ($tous_les_joueurs as $joueur) {
        if ($joueur['total_motm'] > 0) { $hall_of_fame[] = $joueur; }
        $nom_complet = $joueur['prenom'] . ' ' . $joueur['nom'];
        if (in_array($nom_complet, $liste_titulaires)) {
            $equipe[$joueur['poste']][] = $joueur;
        } else {
            $remplacants[] = $joueur;
        }
    }
    usort($hall_of_fame, function($a, $b) { return $b['total_motm'] - $a['total_motm']; });

    // 5. Moyennes des notes
    $moyennes = [];
    $reqMoy = $ConnexionBDD->prepare("SELECT id_joueur, ROUND(AVG(note), 1) as moyenne FROM Votes WHERE id_match = ? AND note IS NOT NULL GROUP BY id_joueur");
...
<head>
    ...
    <style>
        ...
        .player-card { background: white; color: #333; border-radius: 12px; padding: 5px; width: 90px; cursor: pointer; position: relative; margin-bottom: 5px; transition: transform 0.2s; }
...
<p id="statut-vote" style="margin-top: 20px;">
    ...
    <?php else: ?>
        <span style="color: #e74c3c; font-weight: bold;">🔴 VOTES FERMÉS (10h00 - 17h00)</span>
    <?php endif; ?>
</p>
